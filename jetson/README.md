...
```bash
docker compose run --rm train --epochs 30 --lr 0.001 --weight-decay 0.01 --width 1024 --num-blocks 3 --train-rollout-iter 10 --eval-max-rollout-iter 100 --batch-size 32 --max-samples 10000 --min-rating 0 --max-rating 0 --rollout-mode categorical --train-init noisy-gt
```
