...
def test_curriculum_state_from_history(tmp_path: Path) -> None:
    ...
    logit = 0.0
    logit = update_curriculum_p_gt_cap_logit(logit, 0.3)
    logit = update_curriculum_p_gt_cap_logit(logit, 0.7)
    assert state.p_gt_cap_by_group()[0] == pytest.approx(curriculum_p_gt_cap_from_logit(logit))
...
def test_last_checkpoint_roundtrip_for_resume(tmp_path: Path) -> None:
    ...
    assert ckpt["curriculum_p_gt_cap"] == pytest.approx(curriculum_state.mean_p_gt_cap())
